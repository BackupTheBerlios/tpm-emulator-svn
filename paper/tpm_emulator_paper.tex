...
(see, e.g., those from~\cite{Sarmenta}) and vendor extensions before
including them in a hardware specification or even before the real
development process starts. Due to the fast simulation in software, e.g.
by using sophisticated algorithms from third party libraries like
GMP~\cite{GMP}, extensive preliminary tests can be performed much more
efficiently.
...
those provided by TrouSerS~\cite{trousers} (package testsuite),
jTSS~\cite{jTSS} (), IBM DAA Test Suite~\cite{ibmdaatest} and
