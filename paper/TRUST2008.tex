...
% The idea behind this process of integrity measurement
...
be unloaded.
% Consequently, a parent key has to be loaded into the TPM before
...
% arbitrary data) are never disclosed to the user. Furthermore,
Each key is either marked as being \emph{migratable} or \emph{non-migratable}.
...
is never duplicated.
% Due to the limited size of data that can be directly
...
% of objects,
Furthermore, the TPM specification distinguishes between \emph{binding} and
...
hardware-based or a algorithmic pseudo random-number generator; %as the source
% of randomness for the generation of nonces, keys, and padding masks;
...
where most data is reset to its default value. In mode state, which
...
Some technical issues that occurred during implementation are worth to be noted
here: First of all, initially the TPM emulator was designed as a sole kernel
module. Thus, implementation and debugging of most functions (in particular
those needed for the RSA support) was not straightforward due to the limited
kernel stack size (4~KByte or 8~KByte on~x86) and the non-availability of
third party libraries. Moreover, the persistent storage needed for saving the
internal state of the TPM was obtained by directly accessing the local file
system from the kernel module. This direct access can cause unintended side
effects if the user context changes and should thus be avoided. All these issues
have been solved in our new design by introducing a user-space daemon. Finally,
the typographical errors and ambiguities in early revisions of the TPM
specification~\cite{TPM} often turned out to be additional stumbling blocks
during development.
...
evaluation, the experiments were conducted on a IBM ThinkPad T43p (Pentium M @
...
Currently the following function blocks are still missing: some capability
areas or commands as well as all migration and delegation functions.
...
\tpmcmdg{TPM\_FieldUpgrade}{1.1}{170}{O}{tpm\_management.c}{}\\
\tpmcmdg{TPM\_SetRedirection}{1.1}{154}{O}{tpm\_management.c}{}\\
\tpmcmdg{TPM\_ResetLockValue}{1.2}{64}{M}{tpm\_management.c}{}
...
\tpmcmdg{TPM\_Sealx}{1.2}{61}{O}{tpm\_storage.c}{passed \cite{trousers}}
...
\tpmcmdg{TPM\_CreateMaintenanceArchive}{1.1}{44}{O}{tpm\_maintenance.c}{}\\
\tpmcmdg{TPM\_LoadMaintenanceArchive}{1.1}{45}{O}{tpm\_maintenance.c}{}\\
\tpmcmdg{TPM\_KillMaintenanceFeature}{1.1}{46}{O}{tpm\_maintenance.c}{}\\
\tpmcmdg{TPM\_LoadManuMaintPub}{1.1}{47}{O}{tpm\_maintenance.c}{}\\
\tpmcmdg{TPM\_ReadManuMaintPub}{1.1}{48}{O}{tpm\_maintenance.c}{}
